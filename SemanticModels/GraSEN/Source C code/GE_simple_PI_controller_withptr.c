 ...
 */
double PI_calc(double *integrator_state, double Input_dmd, double Input_sensed, double Kp_M, double Ki_M, double timestep)
{
...
int main(int argc, char **argv)
{
    ...
    double integrator_state = 0;

    sensed_output = 0.0;

    for (int i = 0; i < num_steps; i++)
    {
        plant_command = PI_calc(&integrator_state, desired_output, sensed_output, Kp_M, Ki_M, time_step);
