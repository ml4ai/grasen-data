...
double PI_calc2(double Input_dmd, double Input_sensed, double Kp_M, double Ki_M, double timestep)
{
    double error = Input_dmd - Input_sensed;
  ...
  for (int i = 0; i < num_steps; i++)
    {
	  ...
	  plant_command = PI_calc2(desired_output, sensed_output, Kp_M, Ki_M, time_step);
